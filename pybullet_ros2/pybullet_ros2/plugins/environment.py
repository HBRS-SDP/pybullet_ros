...
import rclpy
import pybullet_ros2.sdf.sdf_parser as sdf_parser


class Environment:
    def __init__(self, pybullet, node, **kargs):
    ...
    def load_environment(self):
        ...
        #rclpy.logwarn('GAZEBO_MODEL_PATH environment variable not set, models will not be able to load...')
        # continue blue console output
        # print('\033[34m')
        #world_path = rclpy.get_param('~environment', None)
        # if not world_path:
        #rclpy.logwarn('environment param not set, will run pybullet with empty world')
        # return
        # if not os.path.isfile(world_path):
        #rclpy.logwarn('environment file not found, world will not be loaded : ' + world_path)
        # continue blue console output
        # print('\033[34m')
        # else:
        #rclpy.loginfo('loading world : ' + world_path)
        ...
        #rclpy.loginfo('loading model : ' + model.filename)
        # try:
        # self.pb.loadSDF(model.filename)
        # except:
        #rclpy.logwarn('failed to load model : ' + model.filename + ', skipping...')
